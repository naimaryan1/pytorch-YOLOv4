...
def transform_to_onnx(weight_file, batch_size, n_classes, IN_IMAGE_H, IN_IMAGE_W):
    ...
    if dynamic:
        x = torch.randn(
            (1, 3, IN_IMAGE_H, IN_IMAGE_W),
            requires_grad=True
        )

        onnx_file_name = (
            "yolov4_-1_3_{}_{}_dynamic.onnx"
            .format(IN_IMAGE_H, IN_IMAGE_W)
        )
        ...
        x = torch.randn(
            (batch_size, 3, IN_IMAGE_H, IN_IMAGE_W),
            requires_grad=True
        )

        onnx_file_name = (
            "yolov4_{}_3_{}_{}_static.onnx"
            .format(batch_size, IN_IMAGE_H, IN_IMAGE_W)
        )
...
if __name__ == '__main__':
    ...
    if len(sys.argv) == 8:
        ...
        print(
            ...
            '<weight_file> <image_path> <batch_size> '
